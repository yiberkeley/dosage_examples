...
tmle_ltmle <- function(data, shift_type = "static", alpha = 0.001,
  ...
  SL.library <- list("SL.glm", "SL.mean", "SL.earth")
...
run_tmle_simulation <- function(n_sim = 500, n_obs = 500, tau = 10,
                                ...
                                mc_size = 50000,
                                gbounds = c(0.01, 0.99),
                                SL.library.g = NULL) {  # Added parameter
...
calculate_oracle_coverage_detailed <- function(sim_results) {
  if(length(sim_results) == 0) {
    cat("No results to calculate oracle coverage\n")
    return(NULL)
  }
  ...
  # Calculate empirical SE for each method
  empirical_ses <- apply(estimates, 2, sd, na.rm = TRUE)
  
  # Initialize results vectors
  oracle_coverage <- numeric(length(method_names))
  oracle_ci_width <- numeric(length(method_names))
  se_ratio <- numeric(length(method_names))
  ...
  for(j in 1:length(method_names)) {
    ...
    if(sum(valid_rows) > 0 && !is.na(empirical_ses[j])) {
      # Calculate oracle CI bounds
      oracle_ci_lower <- estimates[valid_rows, j] - 1.96 * empirical_ses[j]
      oracle_ci_upper <- estimates[valid_rows, j] + 1.96 * empirical_ses[j]
      
      # Coverage
      coverage_indicators <- (oracle_ci_lower <= truths[valid_rows, j]) & 
        (oracle_ci_upper >= truths[valid_rows, j])
      oracle_coverage[j] <- mean(coverage_indicators)
      
      # CI width (constant for oracle)
      oracle_ci_width[j] <- 2 * 1.96 * empirical_ses[j]
      
      # Ratio of mean reported SE to empirical SE
      se_ratio[j] <- mean(ses[valid_rows, j], na.rm = TRUE) / empirical_ses[j]
    } else {
      oracle_coverage[j] <- NA
      oracle_ci_width[j] <- NA
      se_ratio[j] <- NA
    }
  }
  ...
  results <- data.frame(
    ...
    Mean_Reported_SE = colMeans(ses, na.rm = TRUE),
    SE_Ratio = se_ratio,
    Oracle_Coverage = oracle_coverage,
    Oracle_CI_Width = oracle_ci_width,
    N_Valid = colSums(!is.na(estimates) & !is.na(truths))
  )
...
set.seed(123)
SL.lib.g <- c("SL.glm", "SL.earth", "SL.mean")  # Or more advanced library
