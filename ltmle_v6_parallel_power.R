...
# genData_10tp_modified <- StrongSignalDGP
...
full_results <- run_tmle_simulation_bootstrap_parallel(
  n_sim = 1000,
  n_obs = 500,
  alphas = c(0,0.001, 0.005, 0.01, 0.02, 5/(sqrt(500)*log(500)), 0.05, 0.10, 0.20, 0.40, 0.60, 1),
  mc_size = 50000,
  gbounds = c(0.001, 0.999),
  SL.library.g = SL.lib.g,
  n_bootstrap = 1, #SET n_boostrap=1 if we want to simply have oracle behavior. Otherwise, set it to 200 for example.
  n_cores = NULL  # Will automatically use detectCores() - 1
)
full_summary <- summarize_tmle_results(full_results)

# Save all results
saveRDS(full_results, 'tmle_ltmle_all_ci_methods_results_seed0920_weak.rds')
write.csv(full_summary, 'tmle_ltmle_all_ci_methods_summary_seed0920_weak.csv')
